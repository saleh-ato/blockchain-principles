...
class ConnectionPool:
    ...
    def broadcast(self, writer, message):
        """general message to the entire pool"""
    ...
    def broadcast_user_quit(self,writer):
        """calls broadcast method with a 'user left' message"""
        self.broadcast(writer, f"{writer.nickname} just quit")

    def broadcast_new_message(self, writer, message):
        """calls broadcast method with user's chat message"""
...
async def handle_connection(reader, writer):
    ...
    while True:
        ...
        if writer.is_closing():
            break
    #we are out of message loop now cleaning
    writer.close()
